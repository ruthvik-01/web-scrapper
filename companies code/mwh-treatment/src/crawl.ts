  ...
  try {
    while (queue.length && requests < maxPages) {
      ...
      try {
        ...
        const board = !item.seed && atsBoard(url);
          ...
          try {
            ...
            while (requests < maxPages) {
              ...
              for (const job of batch) {
                if (usefulDetail(job, now) && job.jobUrl) {
                  if (job.jobUrl === url) {
        ...
        for (let step = 0; step <= maxInteractions; step++) {
          ...
          const control = selector ? current.locator(selector).first() :
            current.getByRole("button", { name: /^(?:load more|show more|more jobs|load more jobs|show more jobs|next(?: page)?)$/i }).first();
