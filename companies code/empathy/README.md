...
Job exports and scrape-report.json are under `runs/<timestamp>/empathy/`. Review the report even if no jobs are exported. Exit code 2 signals partial, failed or unsupported extraction; unexpected errors also return a nonzero code.
...
User-requested policy: postedDate uses time_updated, not publication; two-month filtering uses update dates. Check workplace_type against location.is_remote. Employment type only from explicit source fields. User-supplied website: https://empathy.com/. Scope follows supplied Comeet board.
