...
Job exports and scrape-report.json are under `runs/<timestamp>/news-uk/`. Review the report even if no jobs are exported. Exit code 2 signals partial, failed or unsupported extraction; unexpected errors also return a nonzero code.
...
Configuration source: `output/combined 17-9-26 batches/code/news-uk/company.json`.
