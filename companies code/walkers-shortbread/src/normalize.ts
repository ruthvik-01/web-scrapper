...
export function normalizeJobs(jobs: RawJob[], now = new Date()): { rows: JobRow[]; skipped: SkippedJob[]; dateFallbacks: DateFallback[]; dataNotes: DataNote[] } {
  ...
  for (const job of jobs) {
    // User-requested policy: an ABSENT source date uses this run's UK calendar day.
    // A present but invalid/ambiguous date must not silently become today's date.
    const missingDate = !text(job.postedDate) || /^(null|n\/?a|not specified)$/i.test(text(job.postedDate));
    const postedDate = missingDate ? window.to : parsePostedDate(job.postedDate, now);
    ...
    const reason: SkippedJob["reason"] | undefined =
      ...
      !postedDate ? "unknown_date" :
      postedDate < window.from || postedDate > window.to ? "outside_date_window" :
