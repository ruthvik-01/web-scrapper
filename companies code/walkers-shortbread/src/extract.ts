...
export function jobsFromJson(value: unknown, pageUrl: string, company = ""): RawJob[] {
  const jobs: RawJob[] = [];
  function visit(node: unknown, depth: number): void {
    ...
    if (array(data["@type"]).some(type => /(?:^|[/#])JobPosting$/.test(text(type)))) {
      jobs.push(schemaJob(data, pageUrl, company));
      return;
    }
...
function selectorJobs(html: string, url: string, selectors: Selectors, company: string): RawJob[] {
  ...
  return containers.map(container => {
    ...
    return {
      ...
      description: field($, selectors.description, true), jobUrl: canonicalUrl(link || url, url),
...
export function extractJobs(html: string, url: string, company = "", selectors?: Selectors): RawJob[] {
  ...
  if ($('a[href*="eploy"]').length && /fcVacancyDetails/.test(html)) {
...
export function discoverLinks(html: string, pageUrl: string, selectors?: Selectors): string[] {
  ...
  const add = (href: string | undefined): void => {
    ...
    if (url && !/\.(?:pdf|zip|png|jpg|svg|mp4|docx?)(?:\?|$)/i.test(url)) links.add(url);
  };
  $("a[href], iframe[src]").each((_, node) => {
    ...
    if (detectAts(absolute) !== "Unknown" || careers.test(href) || careers.test(label) ||
  ...
  for (const selector of [selectors?.jobLinks, selectors?.next]) {
