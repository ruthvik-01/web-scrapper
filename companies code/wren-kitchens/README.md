...
Job exports and scrape-report.json are under `runs/<timestamp>/wren-kitchens/`. Review the report even if no jobs are exported. Exit code 2 signals partial, failed or unsupported extraction; unexpected errors also return a nonzero code.
...
Configuration source: `output/combined 17-9-26 batches/code/wren-kitchens/company.json`.

Replaces Phase Eight in the second batch at the user's request. Public Wren careers sitemap links to canonical wrenweb.eploy.net detail pages; each origin's robots.txt is respected. Live listings with old posting dates remain subject to the two-calendar-month window.
