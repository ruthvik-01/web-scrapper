...
Job exports and scrape-report.json are under `runs/<timestamp>/frasers-hospitality/`. Review the report even if no jobs are exported. Exit code 2 signals partial, failed or unsupported extraction; unexpected errors also return a nonzero code.
...
Configuration source: `output/combined 17-9-26 batches/code/frasers-hospitality/company.json`.

User-supplied Malmaison/Hotel du Vin portal. Original hiring-brand names are retained in source rows and reports; the exported company label is Frasers Hospitality.
