...
const workArrangement = (value: string): boolean => /^(hybrid|office(?:[- ]based)?|community(?:[- ]based)?)$/i.test(value);
...
export class Geography {
  ...
  async resolve(job: RawJob): Promise<RawJob> {
    if (job.ats !== "Eploy" && job.ats !== "WP Job Manager") return job;
    const source = job.locations.map(location => ({ ...location }));
    const notes = [...(job.notes || [])];
    const visibleLabel = text(job.visibleLocation);
    // "All Locations" sometimes contains a working arrangement, not a place.
    // It does not contradict the JobPosting's role-specific address.
    const label = workArrangement(visibleLabel) ? "" : visibleLabel;
    if (visibleLabel && !label) notes.push(`Visible label "${visibleLabel}" is a work arrangement, not a geographic place; retain only source-address geography.`);
    ...
    // Only a role's explicitly labelled location line, never footer/contact addresses.
    const roleLocation = /^\s*(?:Location|Work location|Based at)\s*:\s*([^\n]+)/im.exec(job.roleDescription || "")?.[1];
    const rolePostcode = roleLocation && /\b[A-Z]{1,2}\d[A-Z\d]?\s*\d[A-Z]{2}\b/i.exec(roleLocation)?.[0];
    if (source.length === 1 && !source[0]!.postcode && rolePostcode) {
      source[0]!.postcode = rolePostcode;
    ...
    try {
      ...
      const sourceEvidenceStates = unique(source.filter(location => !location.resolved).map(location => text(location.state)));
      ...
      if (!uk) {
        for (const matches of exact.values()) {
          if (matches.some(place => sourceEvidenceStates.some(state =>
      ...
      this.evidence.push({
        jobId: job.jobId || "", jobUrl: job.jobUrl, visibleLocation: visibleLabel,
